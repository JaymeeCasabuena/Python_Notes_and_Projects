...
class Hat:
    def __init__(self, **kwargs):
        self.contents = [color for color, count in kwargs.items() for x in range(count)]
...
def experiment(hat, expected_balls, num_balls_drawn, num_experiments):
    ...
    for x in range(num_experiments):
        ...
        drawn = Counter(hat_copy.draw(num_balls_drawn))
        if all(expected_balls[item] <= drawn.get(item, 0) for item in expected_balls):
            count_m += 1
...
# print(hat.draw(4))
print(experiment(hat, expected_balls={'red':2,'green':1}, num_balls_drawn=5, num_experiments=2000))